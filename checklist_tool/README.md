...
├── 配置核查表_v2.0.0_标注自动验证.xlsx      检查项 × 10 类核查对象矩阵（第13列标注工具自动验证状态）
└── 指导书与核查工具交叉验证报告.docx         指导书与脚本逐项比对（136 项全覆盖，自动化率 63.2%）
```
...
- 《指导书与核查工具交叉验证报告》—— 已实现 86 项 / 部分实现 50 项 / 未实现 0 项（第 5 章 23 项：11 项自动判定、3 项部分信号、9 项需人工）
