...
Sub AddManualChecks()
    ...
    ' ---------- 不适用项（网络设备/其他设备类）----------
    Call AddResult("5.1","网络安全","JD网络跨网跨域数据交换时应按规定流程进行","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.1节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.2","网络安全","利用无线网络技术构建高防护等级网络时应按规定流程执行","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.2节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.3","网络安全","应按最小化原则设计网络架构","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.3节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.4","网络安全","网络边界物理互联节点、路由协议与路由地址网段应满足最小化原则","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.4节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.5","网络安全","局域网内部应根据业务性质划分安全区域","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.5节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.6","网络安全","网络设备应按最小化原则进行远程管理、账户设置、访问控制等安全配置","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.6节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.7","网络安全","网络设备应开放唯一网络管理服务并限制管理终端访问","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.7节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.8","网络安全","远程管理网络设备与安全防护设备应采用加密保护的管理服务","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.8节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.9","网络安全","同链路相同安全功能的防护设备应使用不同架构或不同品牌","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.9节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.10","网络安全","应对组播源、组播地址、组播成员采取控制措施","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.10节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.11","网络安全","重要网络设备和网络安全防护设备应有备份","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.11节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.12","网络安全","远程传输应采取两层加密保护措施","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.12节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.13","网络安全","应设立网络安全管理中心并指定管理终端","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.13节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.14","网络安全","与互联网等外部网络应按等级采取物理或逻辑隔离","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.14节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.15","网络安全","不同用途网络之间应通过防护设备加强逻辑隔离并具备攻击告警审计阻断能力","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.15节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.16","网络安全","局域网各安全区域之间及主机之间应采取全网细粒度访问控制","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.16节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.17","网络安全","远程租用线路传输应采取三层加密保护措施","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.17节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.18","网络安全","应采取不低于802.1x认证强度的接入认证措施","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.18节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.19","网络安全","用户计算机之间应逻辑隔离","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.19节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.20","网络安全","应具备全网行为审计记录能力且审计日志至少保留180天","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.20节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.21","网络安全","应对全网攻击与违规行为采取实时监视报警审计控制阻断定位措施","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.21节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.22","网络安全","数据存储系统的管理网络与应用网络应逻辑隔离","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.22节","按指导书第5章对应小节，采集设备回显后判定。")
    Call AddResult("5.23","网络安全","检查网络设备配备的合理性与必要性","na","本项属网络设备/网络架构类核查项，需登录网络设备采集配置回显后判定，本机脚本不适用；请用网络设备核查（init → 采集回显 → check）出具结论。","第5章 第5.23节","按指导书第5章对应小节，采集设备回显后判定。")
    ' ---------- 不适用项（应用安全其他项）----------
...
Sub GenerateHTML()
    ...
    ts.WriteLine "<title>配置核查报告（Windows XP/7版）</title>"
    ts.WriteLine "<style>"
