...
A('% 配置核查指导书')
A('% 和工具交叉验证报告')
A('%> 验证基准：《配置核查作业指导书》v2.2（10 章、' + str(total) + ' 个编号检查项）')
A('%> 验证对象：自动化核查工具（Windows 版、麒麟版、人工核查台）')
A('')
A('本报告验证《配置核查作业指导书》所列检查项由自动化核查工具执行的程度：'
  '以指导书全部编号检查项为基准，逐条比对核查工具脚本的核查能力，并附工具实际运行证据。')
A('')
A('| 文档信息 | 内容 |')
A('|:--|:--|')
A('| 文档版本 | V1.0 |')
A(f'| 报告日期 | {today} |')
A('| 基准文档 | 《配置核查作业指导书》v2.2（10 章、' + str(total) + ' 个编号检查项） |')
A('| 验证对象 | 自动化核查工具（Windows 版、麒麟版、人工核查台） |')
...
A('- 项数口径：操作系统核查脚本输出 139 项判定，其中 136 项与指导书编号检查项一一对应，另 3 项（口令双因子认证、日志留存完整性、密钥签发与证书管控）为脚本在编号体系外自行补充的核查条目；本报告统计一律以指导书 136 个编号检查项为基准。')
A('- 工具的组成、部署方式与使用方法详见《配置核查工具操作说明书》。')
A('')
A('### 2.2 验证方法')
A('')
...
A('## 6 验证证据')
A('')
_nf = sum(len(_i) for _, _, _i in FIG_GROUPS)
A(f'以下 {_nf} 张截图取自核查工具在实际环境运行的输出，与第 1 章至第 3 章的验证结论逐节对应：'
  '6.1～6.3 为脚本运行过程实况，6.4～6.5 为生成的核查报告，6.6 为人工核查与汇总，'
  '6.7 为抽查检查项的判定与实测配置对照。')
A('')

_figno = 0
for _g, _note, _items in FIG_GROUPS:
    _n0, _n1 = _figno + 1, _figno + len(_items)
    A(f'### {_g}')
    A('')
    A(GROUP_LEADS.get(_g, '').format(n0=_n0, n1=_n1))
    A('')
    if _note:
        A(_note)
        A('')
    for _fn, _cap in _items:
        _figno += 1
        A(f'![图 {_figno}　{_cap}](验证截图/{_fn}.png)')
        A('')
