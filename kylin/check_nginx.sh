...
# 判断 nginx 版本是否过旧（<1.24 视为 EOL/过旧）
nginx_eol() {
    local v="$1" mm
    mm="$(echo "$v" | grep -oE '^[0-9]+\.[0-9]+' | head -1)"
    if [ -n "$mm" ] && [ "$(echo "$mm" | awk -F. '{print $1}')" -lt 1 ] 2>/dev/null; then echo "eol"; return; fi
    local mi; mi="$(echo "$mm" | awk -F. '{print $2}')"
    [ "$mi" -lt 24 ] 2>/dev/null && echo "eol" || echo "supported"
}
...
generate_html() {
    local outfile="$OUT_DIR/配置核查报告_Nginx_${STAMP}.html"
    local pass=0 fail=0 manual=0 na=0 i
    for ((i=1; i<=R_COUNT; i++)); do
        case "${R_STATUS[$i]}" in pass) pass=$((pass+1));; fail) fail=$((fail+1));; manual) manual=$((manual+1));; na) na=$((na+1));; esac
    done
...
<body>
<h1>Nginx 中间件配置核查报告</h1>
<div class="meta">
<div>版本：$(html_esc "${NGINX_VERSION:-未检测到}")　主机名：$(hostname 2>/dev/null)　核查时间：$(date '+%Y-%m-%d %H:%M:%S')</div>
...
generate_xls() {
    local outfile="$OUT_DIR/配置核查报告_Nginx_${STAMP}.xls"
    local pass=0 fail=0 manual=0 na=0 i
    for ((i=1; i<=R_COUNT; i++)); do
        case "${R_STATUS[$i]}" in pass) pass=$((pass+1));; fail) fail=$((fail+1));; manual) manual=$((manual+1));; na) na=$((na+1));; esac
    done
...
<body>
<p>Nginx 中间件配置核查报告　版本：$(html_esc "${NGINX_VERSION:-未检测到}")　核查时间：$(date '+%Y-%m-%d %H:%M:%S')</p>
...
check_1_1_patch() {
    local eol; eol="$(nginx_eol "$NGINX_VERSION")"
    if [ "$eol" = "eol" ]; then
        add_result "1.1" "系统安全" "中间件补丁程序" "fail" "Nginx 当前版本 ${NGINX_VERSION} 过旧（<1.24），存在未修复安全漏洞风险。" "第1章" "升级到 Nginx 1.24/1.26 稳定版并安装最新补丁。"
    else
        add_result "1.1" "系统安全" "中间件补丁程序" "manual" "Nginx 当前版本 ${NGINX_VERSION}，请人工核对是否为最新稳定版补丁。" "第1章" "及时升级到最新稳定版本。"
    fi
}
...
check_1_14_hardening() {
    ...
    # 1) worker 是否降权运行
    local user_line
    user_line="$(grep -E '^[[:space:]]*user[[:space:]]' "$NGINX_CONF" 2>/dev/null | head -1)"
    echo "$user_line" | grep -qE 'user[[:space:]]+root' && issues="${issues}worker以root运行；"
    # 2) 是否配置 HTTPS 管理通道
    local ssl
    ssl="$(grep -rE 'listen[[:space:]]+443[[:space:]]+ssl|ssl_certificate[[:space:]]' /etc/nginx/ 2>/dev/null | head -3)"
    [ -z "$ssl" ] && issues="${issues}未配置HTTPS；"
    # 3) 是否隐藏版本号
    grep -qE 'server_tokens[[:space:]]+off' "$NGINX_CONF" 2>/dev/null || issues="${issues}未隐藏版本号(server_tokens off)；"
    # 4) 是否开启目录浏览
    grep -qE 'autoindex[[:space:]]+on' "$NGINX_CONF" 2>/dev/null && issues="${issues}开启目录浏览(autoindex on)；"
    if [ -z "$issues" ]; then
        add_result "1.14" "系统安全" "中间件安全加固" "pass" "Nginx 已做基础安全加固（worker 降权、配置 HTTPS、隐藏版本号、关闭目录浏览）。" "第1章" "持续保持中间件安全配置。"
    else
        add_result "1.14" "系统安全" "中间件安全加固" "fail" "Nginx 存在以下安全加固缺失：$issues" "第1章" "按缺失项逐条加固中间件配置。"
    fi
}
...
check_4_5_antiddos() {
    local limit
    limit="$(grep -rE 'limit_req|limit_conn' /etc/nginx/ 2>/dev/null | grep -v '#' | head -3)"
    if [ -n "$limit" ]; then
        add_result "4.5" "应用安全" "防DDoS攻击能力" "pass" "已配置请求/连接限速：$(echo "$limit" | head -1)" "第4章" "结合前置防火墙/WAF 持续防护 DDoS。"
    else
        add_result "4.5" "应用安全" "防DDoS攻击能力" "manual" "未检测到 nginx 限速配置（limit_req/limit_conn），防 DDoS 需结合前置防火墙/WAF，请人工核查。" "第4章" "配置限速或前置 WAF/防火墙防 DDoS。"
    fi
}

check_4_6_waf() {
    local waf
    waf="$("$NGINX_BIN" -V 2>&1 | grep -ioE 'modsecurity|naxsi|lua')"
    if [ -n "$waf" ]; then
        add_result "4.6" "应用安全" "Web防护措施" "pass" "检测到 Web 防护模块：$waf" "第4章" "持续保持 Web 防护规则更新。"
    else
        add_result "4.6" "应用安全" "Web防护措施" "manual" "未检测到 WAF 模块（modsecurity/naxsi/lua），需结合前置 WAF 防护，请人工核查。" "第4章" "部署 WAF 或前置安全设备。"
    fi
}
...
check_4_22_defaultport() {
    local port80
    port80="$(grep -hE 'listen[[:space:]]+80([[:space:]]|;)' "$NGINX_CONF" /etc/nginx/conf.d/*.conf 2>/dev/null | grep -v '#' | head -1)"
    if [ -n "$port80" ]; then
        add_result "4.22" "应用安全" "更改默认服务发布端口" "fail" "仍使用默认 80 端口发布服务：$(echo "$port80" | head -1)" "第4章" "更改默认 80 端口为非默认端口。"
    else
        add_result "4.22" "应用安全" "更改默认服务发布端口" "pass" "未使用默认 80 端口发布服务。" "第4章" "持续保持非默认发布端口。"
    fi
}

check_4_23_mgmtport() {
    add_result "4.23" "应用安全" "管理端口与应用端口分离" "na" "Nginx 无独立管理端口（管理通过操作系统 SSH），此条不适用于 Nginx。" "第4章" "管理通道与应用通道分离。"
}
...
if [ -z "$NGINX_BIN" ]; then
    NGINX_PRESENT=0
    NGINX_VERSION="未检测到nginx"
    echo "  未检测到 nginx，所有项将标记为不适用。"
elif NGINX_VERSION="$("$NGINX_BIN" -v 2>&1 | grep -oE 'nginx/[0-9.]+' | head -1 | sed 's#nginx/##')"; [ -n "$NGINX_VERSION" ]; then
    NGINX_PRESENT=1
    echo "  Nginx 版本：$NGINX_VERSION"
else
    NGINX_PRESENT=1
    echo "  检测到 nginx 但无法获取版本。"
fi
echo ""
echo "开始检查，请稍候..."

if [ "$NGINX_PRESENT" = "1" ]; then
    check_1_1_patch
    check_1_14_hardening
    check_4_5_antiddos
    check_4_6_waf
    ...
    add_result "1.1"  "系统安全" "中间件补丁程序" "na" "未检测到 Nginx，标记不适用。" "第1章" "如部署 Nginx 需及时打补丁。"
    add_result "1.14" "系统安全" "中间件安全加固" "na" "未检测到 Nginx，标记不适用。" "第1章" "如部署 Nginx 需做安全加固。"
    add_result "4.5"  "应用安全" "防DDoS攻击能力" "na" "未检测到 Nginx，标记不适用。" "第4章" "如部署 Nginx 需配置限速/前置防护。"
    add_result "4.6"  "应用安全" "Web防护措施" "na" "未检测到 Nginx，标记不适用。" "第4章" "如部署 Nginx 需部署 WAF。"
    add_result "4.8"  "应用安全" "网页防篡改" "na" "未检测到 Nginx，标记不适用。" "第4章" "如部署 Nginx 需配置防篡改。"
    add_result "4.9"  "应用安全" "防范SQL注入/XSS" "na" "未检测到 Nginx，标记不适用。" "第4章" "如部署 Nginx 需应用层防护。"
    add_result "4.22" "应用安全" "更改默认服务发布端口" "na" "未检测到 Nginx，标记不适用。" "第4章" "如部署 Nginx 需改默认端口。"
    add_result "4.23" "应用安全" "管理端口与应用端口分离" "na" "未检测到 Nginx，标记不适用。" "第4章" "管理通道与应用通道分离。"
fi
...
generate_html
generate_xls
type generate_xlsx >/dev/null 2>&1 && generate_xlsx "$OUT_DIR/配置核查报告_Nginx_${STAMP}.xlsx" "Nginx 中间件配置核查报告"
echo ""
