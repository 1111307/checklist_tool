...
# ---------- 结果存储（并行数组）----------
R_ID=(); R_CAT=(); R_TITLE=(); R_STATUS=(); R_DETAIL=(); R_CHAPTER=(); R_REC=()
R_COUNT=0
...
HTMLHEAD
        if [ "$fail" -gt 0 ]; then
            echo "<h2>一、未通过（$fail 项）</h2><table><tr><th>章节</th><th>编号</th><th>类别</th><th>核查项</th><th>结果</th><th>详情</th><th>建议</th></tr>"
            build_table_rows_by_status fail
            echo "</table>"
        fi
        if [ "$manual" -gt 0 ]; then
            echo "<h2>二、需人工核查（$manual 项）</h2><table><tr><th>章节</th><th>编号</th><th>类别</th><th>核查项</th><th>结果</th><th>详情</th><th>建议</th></tr>"
            build_table_rows_by_status manual
            echo "</table>"
        fi
        if [ "$na" -gt 0 ]; then
            echo "<h2>三、不适用（$na 项）</h2><table><tr><th>章节</th><th>编号</th><th>类别</th><th>核查项</th><th>结果</th><th>详情</th><th>建议</th></tr>"
            build_table_rows_by_status na
            echo "</table>"
        fi
        if [ "$pass" -gt 0 ]; then
            echo "<h2>四、通过（$pass 项）</h2><table><tr><th>章节</th><th>编号</th><th>类别</th><th>核查项</th><th>结果</th><th>详情</th><th>建议</th></tr>"
...
XLSHEAD
        if [ "$fail" -gt 0 ]; then
            echo "<p><b>一、未通过（$fail 项）</b></p><table><tr><th>章节</th><th>编号</th><th>类别</th><th>核查项</th><th>结果</th><th>详情</th><th>建议</th></tr>"
            build_table_rows_by_status fail
            echo "</table>"
        fi
        if [ "$manual" -gt 0 ]; then
            echo "<p><b>二、需人工核查（$manual 项）</b></p><table><tr><th>章节</th><th>编号</th><th>类别</th><th>核查项</th><th>结果</th><th>详情</th><th>建议</th></tr>"
            build_table_rows_by_status manual
            echo "</table>"
        fi
        if [ "$na" -gt 0 ]; then
            echo "<p><b>三、不适用（$na 项）</b></p><table><tr><th>章节</th><th>编号</th><th>类别</th><th>核查项</th><th>结果</th><th>详情</th><th>建议</th></tr>"
            build_table_rows_by_status na
            echo "</table>"
        fi
        if [ "$pass" -gt 0 ]; then
            echo "<p><b>四、通过（$pass 项）</b></p><table><tr><th>章节</th><th>编号</th><th>类别</th><th>核查项</th><th>结果</th><th>详情</th><th>建议</th></tr>"
            build_table_rows_by_status pass
            echo "</table>"
        fi
        cat <<XLSFOOT
