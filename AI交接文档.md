...
### 排版全面对齐指导书 + 简称口径（2026-09-15，commit 08eea3b / 456cfef）

用户多轮反馈"行距照抄指导书""正文别黑体""段落不要额外空""不要说该工具"，最终定位与修复：

1. **根因（从第一天就在）**：python-docx 默认模板 docDefaults 带 `段后 10pt + 1.15 倍行距`，作用于所有未显式设置的段落（封面空行、正文段、表格单元格）；指导书 docDefaults 为空。→ md2docx 启动时清零 docDefaults 段距（after=0、去 line/lineRule）。
2. **行距照抄指导书**：指导书正文 14pt + 文档网格 `linePitch=312`（15.6pt/格，每行吸 2 格）→ 实测行距 31.2pt。md2docx 对齐：**BODY_SIZE 12→14pt**、三节全部 `_set_doc_grid(linePitch=312)`、正文两端对齐、去掉显式 1.5 行距（行距由网格决定）。
3. **封面**：结构照抄指导书封面（5 空行→42pt 标题两行→15 空行→校名『西北工业大学制』22pt→5 空行→说明行→1 空行→日期）；md2docx 封面语法 `%#` 校名、`%$` 日期。说明行补入网络设备与华为交换机；报告验证对象行"配置核查工具（脚本、网络设备核查、人工核查台）"保持单行。
4. **简称口径**：说明书与报告首次出现处定义"配置核查工具（以下简称工具）"，正文一律"工具"（原 该工具/本工具/自动化核查工具/核查工具 共 30 余处）；文档信息表、封面、文档标题保留全称。
5. **黑体问题**：WPS 把"宋体+加粗"替换显示为黑体字形 → 正文加粗清零、表题宋体不加粗；正文 14pt 宋体纯排。
6. **坑与教训**：①python-docx 模板 docDefaults 的隐藏段距，用户说"段落额外空"先查 docDefaults；②grid 下封面空行高度按网格吸，需要按指导书实际空行数照抄结构而不是自己调；③**全局字符串替换会误伤文件名**（核查工具→工具 把生成器输出路径改错，静默写到错误文件）——批量替换后必须 diff 全部改动；④验证新增内容存在之后还要验证顺序（4.x 引导句/表题曾颠倒）。
7. 现状态：指导书 705 页、操作说明书 25 页、交叉验证报告 45 页；封面三份同版式单页；正文行距 31.2pt 与指导书一致；半空页 0。
